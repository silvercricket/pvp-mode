...
public class PvPEventHandler
{
    ...
    final long DEFAULTCOOLDOWN = 1800000;	//the time between when each toggle can be made (N/3 if its currently off)
    ...
    public void onLivingUpdate (LivingUpdateEvent event)
    {
        ...
        //Here we need a place to check if the requisite time has gone by 

        //target = getCommandSenderAsPlayer (sender);
        long serverTime=MinecraftServer.getSystemTimeMillis (); 
        long remainingTime=player.getEntityData ().getLong("PvPCooldown");
        //long playerTime=target.getEntityData ().getLong("PvPCooldown"); 
        
        
        
        //player.getEntityData ().setLong("PvPCooldown",player.getEntityData ().getLong("PvPCooldown")-20);        
        	...
        	if (pvptime != 0 && pvptime < MinecraftServer.getSystemTimeMillis ())
        	{
        		...
        		if (player.getEntityData ().getBoolean ("PvPDenied"))
        		{
        			...
        			player.getEntityData ().setLong("PvPCooldown",MinecraftServer.getSystemTimeMillis ()+DEFAULTCOOLDOWN);
        		}
        		else
        		{
        			player.getEntityData ().setBoolean ("PvPDenied", true);
        			player.getEntityData ().setLong("PvPCooldown", MinecraftServer.getSystemTimeMillis ()+(DEFAULTCOOLDOWN/3));
